...
int pico_captive_portal_start(portal_server_t *captive_server, pico_prov_credentials_t *credentials) {
    ...
    if (!captive_server->server_pcb) {
        WL_LOGE("pico_captive_portal", "failed to listen on tcp port");
    ...
    WL_LOGI("pico_captive_portal", "web portal server listening on port 80");
...
err_t pico_captive_portal_sent(void *arg, struct tcp_pcb *tpcb, u16_t len) {
    ...
    WL_LOGI("pico_captive_portal", "successfully sent http frame of length %d to client", len);
...
static void get_wifi_login(void *arg) {
    ...
    if (wifi_credentials->ssid[0] == '\0') {
        WL_LOGE("pico_captive_portal", "no ssid extracted");
        return;
    }
    ...
    if (wifi_credentials->password[0] == '\0') {
        WL_LOGE("pico_captive_portal", "no password extracted");
        return;
    }
...
static void get_value(char *in_buffer, char *key, char *out_buffer) {
    ...
    if (chr_ptr == NULL) {
        WL_LOGE("pico_captive_portal", "key \"%s\" not found", key);
        return;
    }
